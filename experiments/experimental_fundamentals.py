...
def time_patch_methods(model, method_names):
    timing_data = {}
...
def log_timing_data(timing_data, mlflow, fold):
    for method_name, data in timing_data.items():
        if data['call_count'] > 0:
            mlflow.log_metric(f'{method_name}_total_time_fold_{fold}', data['total_time'], step=fold)
            mlflow.log_metric(f'{method_name}_calls_fold_{fold}', data['call_count'], step=fold)
            mlflow.log_metric(f'{method_name}_avg_time_fold_{fold}', data['total_time'] / data['call_count'], step=fold)
